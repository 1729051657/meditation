...
import { SearchHistoryVO, SearchHistoryForm, SearchHistoryQuery } from '@/api/meditation/searchHistory/types';
...
export const listSearchHistory = (query?: SearchHistoryQuery): AxiosPromise<SearchHistoryVO[]> => {
  return request({
    url: '/meditation/searchHistory/list',
    method: 'get',
    params: query
  });
};
...
export const getSearchHistory = (id: string | number): AxiosPromise<SearchHistoryVO> => {
  return request({
    url: '/meditation/searchHistory/' + id,
    method: 'get'
  });
};
