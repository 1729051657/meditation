    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['meditation:tag:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['meditation:tag:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['meditation:tag:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['meditation:tag:export']">导出</el-button>
          </el-col>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['meditation:tag:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['meditation:tag:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
import { listTag, getTag, delTag, addTag, updateTag } from '@/api/meditation/tag';
import { TagVO, TagQuery, TagForm } from '@/api/meditation/tag/types';
...
const handleDelete = async (row?: TagVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除内容标签编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delTag(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('meditation/tag/export', {
    ...queryParams.value
