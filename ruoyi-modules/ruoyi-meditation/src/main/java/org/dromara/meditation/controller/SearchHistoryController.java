...
@RestController
@RequestMapping("/meditation/searchHistory")
public class SearchHistoryController extends BaseController {
     ...
     * 导出搜索历史列表
     */
    @SaCheckPermission("meditation:searchHistory:export")
     ...
     * @param id 主键
     */
    @SaCheckPermission("meditation:searchHistory:query")
    @GetMapping("/{id}")
     ...
     * 修改搜索历史
     */
    @SaCheckPermission("meditation:searchHistory:edit")
     ...
     * @param ids 主键串
     */
    @SaCheckPermission("meditation:searchHistory:remove")
     ...
     * 清空当前用户的搜索历史
     */
    @Log(title = "清空搜索历史", businessType = BusinessType.DELETE)
    @DeleteMapping("/clearMy")
    public R<Void> clearMyHistory() {
        return toAjax(searchHistoryService.clearUserHistory());
    }
}
